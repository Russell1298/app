...
_LIVE_SECRET_LABELS = frozenset({
    ...
    "Stripe Live Secret Key", "Stripe Restricted Key",
    "GitHub Personal Access Token", "GitHub App Token",
    "Google OAuth Token", "Facebook Access Token",
...
async def scan_secrets(domain: str) -> SecretScanResult:
    ...
    risk_score = scanner_score(sum(f.penalty for f in deduped), "secrets")
